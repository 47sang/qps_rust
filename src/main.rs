...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // 生成QPS图表
    generate_qps_chart(&stats, &args.output, args.interval)?;
...
/// 读取并解析日志文件
fn parse_nginx_log(log_path: &PathBuf) -> Result<(Vec<DateTime<Local>>, DateTime<Local>, DateTime<Local>), Box<dyn Error>> {
...
fn calculate_qps(
    ...
    for &ts in timestamps {
        ...
        let interval_start = time_min + Duration::seconds(interval_index * interval_seconds.num_seconds());
...
/// 生成QPS图表
fn generate_qps_chart(qps_data: &[(DateTime<Local>, f64)], output: &str, interval: u64) -> Result<(), Box<dyn Error>> {
    println!("正在生成QPS图表...");
    ...
    // 确定图表大小
    let root = BitMapBackend::new(output, (1500, 768)).into_drawing_area();
    root.fill(&WHITE)?;
    
    // 确定Y轴范围
    let max_qps = qps_data.iter().map(|&(_, qps)| qps).fold(0.0/0.0, f64::max);
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption(format!("Nginx 请求数统计 ({}秒间隔)", interval),("SimSun", 30).into_font())
        .margin(10)
    ...
    // 添加数据点
    chart.draw_series(
        qps_data.iter().map(|&(time, qps)| {
            Circle::new((time, qps), 2, BLUE.filled())
        }),
    )?;
